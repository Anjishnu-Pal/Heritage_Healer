...
def process_and_damage_image(img_path):
    ...
    try:
        ...
        # Your script converts to grayscale and back to RGB, creating a 3-channel grayscale image
...
try:
    subprocess.run(['pip', 'install', 'kaggle'], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    print("Please check Kaggle API setup and dataset ID.")
    print("="*50)
    raise # Exit if download fails
...
if not os.path.exists(SOURCE_IMG_FOLDER):
    ...
    raise FileNotFoundError("Source image folder not found.")
...
print("="*50)
print("✅ Cell 1 Complete: Dataset preparation finished!")
print(f"Your training and validation sets are ready in the '{OUTPUT_DIR}' folder.")
print("You can now run Cell 2 to train the diffusion model.")
print("="*50)
